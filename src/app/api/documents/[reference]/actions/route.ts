...
import { addAuditLog, getDocument, updateDocumentStatus } from "@/lib/demo-store";
...
export async function POST(request: Request, { params }: { params: Promise<{ reference: string }> }) {
  try {
    ...
    updateDocumentStatus(document, status, note);
    addAuditLog({ actorId: admin.id, action: `document.${status}`, entityType: "document_request", entityId: document.id, metadata: { reference } });
